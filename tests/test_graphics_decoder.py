...
@pytest.mark.graphics
@pytest.mark.extractor
def test_decode_graphics_frame_accounting_allows_skipped_with_statuses(
    ...
    atlas = Atlas(
        ...
        flags=0,
        frames=[
            AtlasFrame(index=0, record_type=0, x=0, y=0, width=1, height=1),
            AtlasFrame(index=1, record_type=0, x=0, y=0, width=1, height=1),
        ],
        regions=[Region(index=0, kind=0, x=0, y=0, extra=0)],
        animations=[Animation(index=0, kind=0, offset=0)],
        anchors=[],
        extra_quads=[],
        palettes=[AtlasPalette(index=0, fmt=PAL_FMT_ARGB8888, size=2, colors=[0xFF000000, 0xFFFFFFFF])],
        palette_format=PAL_FMT_ARGB8888,
        palette_size=2,
        pixel_format=22018,
    ...
    output = tmp_path / 'out'
    decode_graphics(tmp_path / 'fake.jar', output)

    metadata_json = json.loads(
        (output / 'extracted' / 'sprites' / 'm11_1' / 'chunk_00' / 'metadata.json').read_text(encoding='utf-8')
    )
    manifest_json = json.loads(
        (output / 'extracted' / 'sprites' / 'm11_1' / 'chunk_00' / 'manifest.json').read_text(encoding='utf-8')
    )
    frames_json = json.loads(
        (output / 'extracted' / 'images' / 'm11_1' / 'chunk_00' / 'frames.json').read_text(encoding='utf-8')
    )

    frame_count = metadata_json['frame_count']
    manifest_count = manifest_json['frame_count']
    exported_count = len(frames_json['frames'])
    skipped_count = len(frames_json['skipped_frames'])

    assert frame_count == manifest_count == 2
    assert exported_count == len(manifest_json['frames']) == 1
    assert exported_count + skipped_count == frame_count
    assert all(item['decode_status'] in {'skipped', 'failed_decode'} for item in frames_json['skipped_frames'])
    assert all(item.get('skipped_reason') for item in frames_json['skipped_frames'])
